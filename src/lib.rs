...
static GITHUB_BASE_URL: &str = "https://raw.githubusercontent.com/barudakbaong/irenweb/refs/heads/main/web";
...
async fn tunnel(req: Request, mut cx: RouteContext<Config>) -> Result<Response> {
    ...
    if PROXYKV_PATTERN.is_match(&proxyip) {
        ...
        if proxy_kv_str.is_empty() {
            ...
            let req = Fetch::Url(Url::parse("https://raw.githubusercontent.com/barudakbaong/PList1/refs/heads/main/kvProxyList.json")?);
            let mut res = req.send().await?;
            if res.status_code() == 200 {
